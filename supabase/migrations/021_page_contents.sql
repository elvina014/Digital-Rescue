-- =============================================
-- 021_page_contents.sql
-- CMS(편집 가능한 페이지 콘텐츠) 스키마 + 초기 시드
--
-- 모델:
--   page_contents = (page_key, section_key, content_data JSONB)
--   - page_key   : 'main', 'brand:samsung', 'brand:dell', ...
--   - section_key: 'theme', 'hero', 'symptoms', 'intro' ...
--   - content_data: 해당 섹션이 렌더링에 필요로 하는 임의의 JSON 구조
--
-- 운영:
--   - 사이트(SSG/SSR)는 (page_key, section_key) 로 행을 조회해 렌더링한다.
--   - CMS(/editor) 는 동일 키로 UPSERT 하여 콘텐츠를 갱신한다.
--   - 초기 시드는 src/data/mainPageData.json, src/data/brandLandingData.json
--     의 현재 값을 그대로 채워 넣어, DB 전환 직후에도 사이트가 동일하게 동작.
-- =============================================
...
CREATE TABLE page_contents (
  ...
  CONSTRAINT page_contents_unique_key UNIQUE (page_key, section_key)
);

COMMENT ON TABLE  page_contents              IS 'CMS 편집 가능한 페이지/섹션 콘텐츠. (page_key, section_key) 로 식별.';
COMMENT ON COLUMN page_contents.page_key     IS '페이지 식별자: ''main'', ''brand:samsung'', ''brand:dell'' 등';
COMMENT ON COLUMN page_contents.section_key  IS '섹션 식별자: ''theme'', ''hero'', ''symptoms'', ''intro'' 등';
COMMENT ON COLUMN page_contents.content_data IS '해당 섹션 렌더링에 필요한 임의의 JSON. 컴포넌트 props와 1:1 매핑.';
COMMENT ON COLUMN page_contents.updated_by   IS '마지막으로 편집한 직원 (감사 로그)';

CREATE INDEX page_contents_page_key_idx ON page_contents (page_key);
...
CREATE OR REPLACE FUNCTION page_contents_set_updated_at()
RETURNS TRIGGER AS $$
BEGIN
  NEW.updated_at := now();
  RETURN NEW;
...
CREATE TRIGGER trg_page_contents_updated_at
  ...
  FOR EACH ROW
  EXECUTE FUNCTION page_contents_set_updated_at();
...
-- 읽기: 누구나(익명 포함) — 사이트 SSG/SSR 렌더링이 anon 으로 조회한다.
CREATE POLICY page_contents_select ON page_contents
  FOR SELECT TO anon, authenticated
  USING (true);

-- 쓰기(INSERT/UPDATE/DELETE): ADMIN, MANAGER 만 — get_my_role() 헬퍼는
-- 001_initial_schema.sql 에서 정의됨.
CREATE POLICY page_contents_insert ON page_contents
  FOR INSERT TO authenticated
  WITH CHECK (get_my_role() IN ('ADMIN', 'MANAGER'));

CREATE POLICY page_contents_update ON page_contents
  FOR UPDATE TO authenticated
  USING      (get_my_role() IN ('ADMIN', 'MANAGER'))
  WITH CHECK (get_my_role() IN ('ADMIN', 'MANAGER'));

CREATE POLICY page_contents_delete ON page_contents
  FOR DELETE TO authenticated
  USING (get_my_role() IN ('ADMIN', 'MANAGER'));


-- =============================================
-- 4. 초기 시드 (현재 src/data/*.json 의 값을 1:1 로 이전)
