...
export function ContactForm({
  ...
  //  2) 없으면 URL 기반 자동 주입(defaultBrand / defaultDeviceType)
  //  3) 그것도 없으면 빈 문자열
  const initialBrand =
    state.values?.deviceBrand ?? defaultBrand ?? "";
  const initialDeviceType =
    state.values?.deviceType ?? defaultDeviceType ?? "";
            ...
            <div className="grid gap-5 sm:grid-cols-2">
              ...
              >
                ...
                >
                  ...
                  {/*
                    URL에서 주입된 브랜드가 cf.brands 배열에 없을 때도
...
function DynamicGuide({
  ...
  const guide = cf.dynamicGuides[receiptType];
