...
const config = data.realtimeStatus;
const theme = data.theme;

type StatusItem = (typeof config.statuses)[number];
...
function generateEntries(count: number): FakeEntry[] {
  return Array.from({ length: count }, (_, i) => ({
    id: i,
    name: pickRandom(config.dummyData.names),
    device: pickRandom(config.dummyData.devices),
    symptom: pickRandom(config.dummyData.symptoms),
    status: pickRandom(config.statuses),
  }));
}
...
export function RealtimeStatus() {
  ...
  const refresh = useCallback(() => {
    setEntries(generateEntries(config.rowCount));
    setLastRefresh(new Date());
  }, []);

  useEffect(() => {
    refresh();
    const interval = setInterval(refresh, config.refreshIntervalMinutes * 60 * 1000);
            ...
            <thead>
              ...
              >
                ...
                <th
                  ...
                  style={{ color: theme.textSecondary }}
                >
                  {config.columns.device}
                </th>
                <th
                  className="px-6 py-4 text-xs font-semibold uppercase tracking-wider"
                  style={{ color: theme.textSecondary }}
                >
                  {config.columns.symptom}
                </th>
                <th
                  className="px-6 py-4 text-xs font-semibold uppercase tracking-wider"
                  style={{ color: theme.textSecondary }}
